...
t_int *henon_perform(t_int *w)
{
    ...
    while (n--){
		...
		if(head > 1.0)
		{
			...
			nextx = previousy + 1 - co1 * previousx * previousx;
			nexty = co2 * previousx;
...
{
	x->x_sr = sp[0]->s_sr;
    dsp_add(henon_perform, 8, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec, sp[5]->s_vec, sp[0]->s_n);
}

void henon_tilde_setup(void)
{
    henon_class = class_new(gensym("henon~"), (t_newmethod)henon_new, 0, sizeof(t_henon), 0, 0);

    class_addmethod(henon_class, (t_method)henon_dsp, gensym("dsp"), 0);
    CLASS_MAINSIGNALIN(henon_class, t_henon, x_f);
    class_sethelpsymbol(henon_class, gensym("sigbinops"));
}
